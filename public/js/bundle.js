...
var _default = exports.default = isXHRAdapterSupported && function (config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    request.onabort = function handleAbort() {
      ...
      reject(new _AxiosError.default('Request aborted', _AxiosError.default.ECONNABORTED, _config, request)); // Clean up request
    ...
    request.onerror = function handleError() {
      ...
      reject(new _AxiosError.default('Network Error', _AxiosError.default.ERR_NETWORK, _config, request)); // Clean up request
    ...
    request.ontimeout = function handleTimeout() {
      ...
      reject(new _AxiosError.default(timeoutErrorMessage, transitional.clarifyTimeoutError ? _AxiosError.default.ETIMEDOUT : _AxiosError.default.ECONNABORTED, _config, request)); // Clean up request
...
var displayMap = exports.displayMap = function displayMap(locations) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var map = L.map('map').setView([51.505, -0.09], 13); // Add the tile layer to the map

    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      maxZoom: 19
    }).addTo(map); // Get the locations data from the dataset
    ...
    var customIcon = L.divIcon({
      className: 'marker',
      // Class name for your custom CSS
      html: '',
      // No need for additional HTML; the marker styling comes from CSS
      iconSize: [32, 40],
      // Size of the icon [width, height]
      iconAnchor: [16, 40],
      // Adjust if necessary to align marker correctly
      popupAnchor: [0, -40] // Adjust if necessary to position popup correctly

    }); // Add markers for each location

    locations.forEach(function (loc) {
      L.marker([loc.coordinates[1], loc.coordinates[0]]).addTo(map).bindPopup("<p>".concat(loc.day, ": ").concat(loc.description, "</p>")).openPopup();
    });
  });
};
...
var stripe = Stripe('pk_test_51PgLSl2MJQnEPzJbai0xqdy6bDYXi9XXjfIfIiVkOZ5qABs8aFd8gXxjqgDWT1MQhYrlqHqvx7j9eOyxWfEpPvOu001dT3Nucb');
